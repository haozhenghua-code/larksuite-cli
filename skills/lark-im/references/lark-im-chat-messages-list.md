...
# Get direct messages with a user (user identity only; pass open_id and resolve p2p chat_id automatically)
...
| `--user-id <id>` | One of two | Specify a DM conversation by the other user's open_id (`ou_xxx`); p2p chat_id is resolved automatically. This path requires user identity (`--as user`) |
...
| `--user-id requires user identity (--as user)` | `--user-id` was used with bot identity | Re-run with `--as user`, or provide `--chat-id` directly when using bot identity |
...
3. **For direct messages:** use `--user-id` to resolve the p2p chat automatically instead of looking it up manually, but only with user identity. For bot identity, provide `--chat-id` directly.
